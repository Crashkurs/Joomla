...
function userSave( $option, $uid) {
	...
	if(isset($_POST["password"]) && $_POST["password"] != "") {
		if(isset($_POST["verifyPass"]) && ($_POST["verifyPass"] == $_POST["password"])) {
			...
			if ($vb_config->vb_useextdb == 1) {
				...
				$database2->setQuery("SELECT salt FROM {$vb_config->vb_prefix}user WHERE userid='$row->vb_userid'");
				$database2->loadObject( $vbulletin_fetch );
				$vbulletin_pass = md5(md5($_POST["password"]) . $vbulletin_fetch->salt);
				$database2->setQuery( "UPDATE {$vb_config->vb_prefix}user SET password='$vbulletin_pass' WHERE userid='$row->vb_userid'" );
				if (!$database2->query()) {
					echo "<script> alert('".$database2->getErrorMsg()."'); window.history.go(-1); </script>\n";
					exit();
				}
			} else {
				$database->setQuery("SELECT salt FROM {$vb_config->vb_prefix}user WHERE userid='$row->vb_userid'");
				$database->loadObject( $vbulletin_fetch );
				$vbulletin_pass = md5(md5($_POST["password"]) . $vbulletin_fetch->salt);
				$database->setQuery( "UPDATE {$vb_config->vb_prefix}user SET password='$vbulletin_pass' WHERE userid='$row->vb_userid'" );
				if (!$database->query()) {
					echo "<script> alert('".$database->getErrorMsg()."'); window.history.go(-1); </script>\n";
					exit();
				}
			}
			setcookie( $vb_config->vb_cookie."password", md5( $vbulletin_pass . $vb_config->vb_license ), time() + 43200, "/" );
			$row->password = $vbulletin_pass;
	...
	$database->setQuery( "SELECT * FROM #__vbridge_config WHERE id='1'" );
	$database->loadObject( $vb_config );
	$cookieuserid=$vb_config->vb_cookie."userid";
	if ($vb_config->vb_useextdb == 1) {
		$database2 = new database( $vb_config->vb_dbhost,$vb_config->vb_dbuser,$vb_config->vb_dbpass, $vb_config->vb_dbname, $vb_config->vb_prefix );
		$database2->setQuery( "UPDATE {$vb_config->vb_prefix}user SET email='$row->email' WHERE userid='$row->vb_userid'" );
		if (!$database2->query()) {
			echo "<script> alert('".$database2->getErrorMsg()."'); window.history.go(-1); </script>\n";
			exit();
		}
	} else {
		$database->setQuery( "UPDATE {$vb_config->vb_prefix}user SET email='$row->email' WHERE userid='$row->vb_userid'" );
		if (!$database->query()) {
			echo "<script> alert('".$database->getErrorMsg()."'); window.history.go(-1); </script>\n";
			exit();
		}
