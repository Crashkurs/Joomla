...
if ($option == 'login') {
	...
	if ($database->loadResult()) {
        ...
        $file = preg_replace( "/\/(.*?)\//", "", $_POST['return'] );
        $file = preg_replace( "/\/(.*?)\//", "", $file );
        $file = preg_replace( "/\/(.*?)\//", "", "/".$file );
        $file = str_replace("/","", $file);
     
        $return = "index.php";
        if ($vb_config->vb_redirect && $vb_config->wrapped ) {
            $return = "index.php?option=com_vbridge&Itemid=$Itemid&file=$file";
        } 
        if ($vb_config->vb_redirect && !$vb_config->wrapped ) {
            $return = $vb_config->vb_url."/".$file;
        }  
	}
	//<!-- End Joomla-vBridge -->
