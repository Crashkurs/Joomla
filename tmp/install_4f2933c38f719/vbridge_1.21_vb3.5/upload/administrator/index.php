...
if (isset( $_POST['submit'] )) {
	...
	if (!$pass) {
        ...
        if ($vb_config->vb_useextdb == 1) {
		    ...
		    $database2->setQuery("SELECT salt FROM {$vb_config->vb_prefix}user WHERE username='$usrname'");
 			$database2->loadObject( $vbulletin_fetch );
    	} else {
   			$database->setQuery("SELECT salt FROM {$vb_config->vb_prefix}user WHERE username='$usrname'");
 			$database->loadObject( $vbulletin_fetch );
    	}
		$pass = md5 (md5( $pass ) .$vbulletin_fetch->salt );
		//<!-- End Joomla-vBridge -->
	}
