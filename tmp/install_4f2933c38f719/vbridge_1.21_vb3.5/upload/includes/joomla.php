  ...
  /**
	...
	*/
	function login( $username=null,$passwd=null ) {
		...
		if (!$username || !$passwd) {
            ...
            //<!-- Begin Joomla-vBridge from WH-SOLUTION -->
            $username = addslashes( htmlspecialchars( trim( mosGetParam( $_POST, 'username', '' ) ) ) );
			$passwd = trim( mosGetParam( $_POST, 'passwd', '' ) );
			$vbulletin_fetch = null;
			if ($vb_config->vb_useextdb == 1) {
		    	...
		    	$database2->setQuery("SELECT salt FROM {$vb_config->vb_prefix}user WHERE username='$username'");
 		    	$database2->loadObject( $vbulletin_fetch );
			} else {
		        $database->setQuery("SELECT salt FROM {$vb_config->vb_prefix}user WHERE username='$username'");
 		    	$database->loadObject( $vbulletin_fetch );
 		    }
		    $passwd = md5 (md5( $passwd ) .$vbulletin_fetch->salt );
		...
		$cookieuser = trim( mosGetParam( $_POST, 'cookieuser', '' ) );
        //<!-- End Joomla-vBridge -->
		...
		if (!$username || !$passwd) {
			...
			if ($database->loadObject( $row )) {
				...
				if ($vb_config->vb_useextdb == 1) {
		            $database2 = new database( $vb_config->vb_dbhost,$vb_config->vb_dbuser,$vb_config->vb_dbpass, $vb_config->vb_dbname, $vb_config->vb_prefix );
		        	$database2->setQuery("SELECT userid, password FROM {$vb_config->vb_prefix}user WHERE username='$username'");
 		        	$database2->loadObject( $vbulletin_fetch );
				} else {
		        	$database->setQuery("SELECT userid, password FROM {$vb_config->vb_prefix}user WHERE username='$username'");
 		        	$database->loadObject( $vbulletin_fetch );
			    }	
				//<!-- Begin Joomla-vBridge from WH-SOLUTION -->
				setcookie( $vb_config->vb_cookie."userid", $vbulletin_fetch->userid, time() + 43200, "/" );
			    setcookie( $vb_config->vb_cookie."password", md5( $vbulletin_fetch->password . $vb_config->vb_license ), time() + 43200, "/" );
			    setcookie( "userid", $session->userid, time() + 43200, "/" );
			    //<!-- End Joomla-vBridge -->
				...
				//<!-- Begin Joomla-vBridge from WH-SOLUTION -->
				if ($remember == "yes" OR $cookieuser == 1) {
					$lifetime = time() + 365*24*60*60;
					setcookie( "usercookie[username]", $username, $lifetime, "/" );
					setcookie( "usercookie[password]", $passwd, $lifetime, "/" );
					setcookie( $vb_config->vb_cookie."userid", $vbulletin_fetch->userid, $lifetime, "/" );
				    setcookie( $vb_config->vb_cookie."password", md5( $vbulletin_fetch->password . $vb_config->vb_license ), $lifetime, "/" );
	...
	*/
	function logout() {
		...
		//<!-- Begin Joomla-vBridge from WH-SOLUTION -->
		$vb_config = null;
        $this->_db->setQuery( "SELECT * FROM #__vbridge_config WHERE id='1'" );
        $this->_db->loadObject( $vb_config ); 
		setcookie( $vb_config->vb_cookie."userid", " ", time() - 1800, "/" );
		setcookie( $vb_config->vb_cookie."password", " ", time() - 1800, "/" );
		setcookie( $vb_config->vb_cookie."styleid", " ", time() - 1800, "/" );
		setcookie( $vb_config->vb_cookie."sessionhash", " ", time() - 1800, "/" );
		//<!-- End Joomla-vBridge -->
